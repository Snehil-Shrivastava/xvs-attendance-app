...
export async function GET() {
  return NextResponse.json(
    {
      status: "active",
      service: "Dahua Biometric Receiver Debug Endpoint",
    },
    { status: 200 },
  );
}
...
export async function POST(req: NextRequest) {
  ...
  console.log("🔥 DAHUA RAW WEBHOOK RECEIVED");
  console.log("receivedAt:", receivedAt);
  console.log("headers:", headers);
  console.log("bodyText:", bodyText);

  try {
    await adminDb.collection("webhook_raw_events").add({
      receivedAt,
      headers,
    ...
    console.log(
      "✅ Raw webhook saved to Firestore collection: webhook_raw_events",
    );
  } catch (error) {
    console.error("❌ Failed to save raw webhook to Firestore:", error);
  }
