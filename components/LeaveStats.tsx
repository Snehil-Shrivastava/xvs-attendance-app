...
//   targetUserId?: string; // <--- ADD THIS PROP
// }

// const LeaveStats = ({ targetUserId }: LeaveStatsProps) => {
//   const { user, userData, loading: authLoading } = useAuth();
//   const effectiveUid = targetUserId || user?.uid; // <--- USE TARGET USER ID

//   const [monthDaysTaken, setMonthDaysTaken] = useState(0);
//   const [yearDaysTaken, setYearDaysTaken] = useState(0);
//   const [explicitUnpaidDays, setExplicitUnpaidDays] = useState(0);
//   const [loadingLeaves, setLoadingLeaves] = useState(true);

//   const now = new Date();
//   const currentYearStr = useMemo(() => String(now.getFullYear()), []);
//   const currentMonthStr = useMemo(() => {
//     const m = String(now.getMonth() + 1).padStart(2, "0");
//     return `${currentYearStr}-${m}`;
//   }, [currentYearStr]);
...
//             const isExplicitUnpaid = data.leaveType === "Unpaid Leave";
...
//               if (start.startsWith(currentYearStr)) {
//                 yearTotal += 0.5;
//                 if (isExplicitUnpaid) unpaidTotal += 0.5;
//               }
//               if (start.startsWith(currentMonthStr)) {
//                 monthTotal += 0.5;
//               }
...
//               if (start.startsWith(currentYearStr)) {
//                 yearTotal += 1;
//                 if (isExplicitUnpaid) unpaidTotal += 1;
//               }
//               if (start.startsWith(currentMonthStr)) {
//                 monthTotal += 1;
//               }
...
//                   yearTotal += Number(data.totalDays || 1);
//                   if (isExplicitUnpaid)
//                     unpaidTotal += Number(data.totalDays || 1);
//                 }
//                 if (start.startsWith(currentMonthStr)) {
//                   monthTotal += Number(data.totalDays || 1);
//                 }
...
//         setMonthDaysTaken(monthTotal);
//         setYearDaysTaken(yearTotal);
//         setExplicitUnpaidDays(unpaidTotal);
//         setLoadingLeaves(false);
//       },
//       (error) => {
//         console.error("Error calculating leave stats:", error);
...
//     return () => unsubscribe();
//   }, [effectiveUid, currentYearStr, currentMonthStr]);
...
//   // 2. Leaves Remaining: YEARLY REMAINING
//   const remainingDays = Math.max(0, annualQuota - yearDaysTaken);
//   const leavesRemainingStr = formatDays(remainingDays);

//   // 3. Unpaid Leaves: Combined (Leaves past 24 quota + any marked as Unpaid Leave)
//   const quotaExceededDays = Math.max(0, yearDaysTaken - annualQuota);
//   const totalUnpaidDays = Math.max(quotaExceededDays, explicitUnpaidDays);
//   const unpaidLeavesStr = formatDays(totalUnpaidDays);
...
//         {/* CARD 2: LEAVES REMAINING (OUT OF 24) */}
//         <div className="bg-[#849F9C] py-2.5 px-2 flex flex-col items-center justify-between text-center text-white gap-1.5">
//           <span className="text-[8px] font-normal tracking-wide opacity-95">
//             Leaves Remaining
//           </span>
...
const LeaveStats = ({ targetUserId }: LeaveStatsProps) => {
  ...
  // Card 1: this month, paid + unpaid
  const leavesTakenStr = formatLeaveDays(summary.monthTotalDays);
  // Card 2: year-remaining (paid bucket)
  const leavesRemainingStr = formatLeaveDays(summary.yearRemaining);
  // Card 3: year unpaid (explicit + quota-exceeded)
